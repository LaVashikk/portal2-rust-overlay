...
use egui::{color_picker, Color32, Context, RichText, Slider};

use crate::engine::{self, IVEngineClient};
use super::{utils::Window, SharedState};
    ...
    fn draw(&mut self, ctx: &Context, shared_state: &mut SharedState) {
        ...
        egui::Window::new(self.name())
            ...
            .show(ctx, |ui| {
                ...
                let client = engine::get().client();
                let cvar_system = engine::get().cvar_system();

                // --- Main Switches ---
                ui.label("Requires sv_cheats 1");
                draw_cvar_checkbox(ui, FOG_OVERRIDE, "Fog Override");
                ui.separator();

                // --- Primary Fog Section ---
                ui.collapsing(RichText::new("Primary Fog").strong(), |ui| {
                    let grid_id = "primary_fog_grid";
                    draw_fog_section(ui, client, grid_id, FOG_ENABLE, FOG_START, FOG_END, FOG_COLOR);
                });
                ui.separator();
                ...
                ui.collapsing(RichText::new("Skybox Fog").strong(), |ui| {
                    ...
                    draw_fog_section(ui, client, grid_id, FOG_ENABLE_SKYBOX, FOG_START_SKYBOX, FOG_END_SKYBOX, FOG_COLOR_SKYBOX);
                });
                ui.separator();
...
fn draw_fog_section(
    ui: &mut egui::Ui,
    _client: &IVEngineClient, // PLACEHOLDER
    grid_id: &str,
    enable_cvar: &str,
    start_cvar: &str,
    end_cvar: &str,
    ...
    let _cvar_system = engine::get().cvar_system();
    draw_cvar_checkbox(ui, enable_cvar, "Enable");
    ...
    egui::Grid::new(grid_id)
        ...
        .show(ui, |ui| {
            // Start
            ui.label("Start:");
            draw_cvar_slider(ui, start_cvar, -1000.0..=10000.0);
            ui.end_row();

            // End
            ui.label("End:");
            draw_cvar_slider(ui, end_cvar, -1000.0..=16000.0);
            ui.end_row();

            // Color
            ui.label("Color:");
            draw_cvar_color_picker(ui, color_cvar);
            ui.end_row();
        });
}
...
fn draw_cvar_checkbox(ui: &mut egui::Ui, name: &str, label: &str) {
    let cvar_system = engine::get().cvar_system();
    let client = engine::get().client();
...
fn draw_cvar_slider(ui: &mut egui::Ui, name: &str, range: std::ops::RangeInclusive<f32>) {
    let cvar_system = engine::get().cvar_system();
    let client = engine::get().client();
...
fn draw_cvar_color_picker(ui: &mut egui::Ui, name: &str) {
    let cvar_system = engine::get().cvar_system();
    let client = engine::get().client();
