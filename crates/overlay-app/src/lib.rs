//! overlay-app: Owns UiManager, global state, WndProc hook, and egui backend glue.
//!
//! Requirements respected:
//! - UiManager is a global singleton (OnceLock<Mutex<...>>).
//! - WndProc hook is NOT inside egui-d3d9; it lives here and cooperates with UiManager.
//! - This crate provides callbacks for d3d9-present-hook.
...
pub use d3d9_present_hook::Callbacks;
pub mod logger;

const TEXT_SCALE: f32 = 1.25;

pub static OVERLAY_APP: OnceLock<Mutex<UiManager>> = OnceLock::new();
static EGUI_RENDERER: OnceLock<Mutex<egui_d3d9::EguiDx9Lite>> = OnceLock::new();
...
/// Overlay controller that owns overlay-ui windows and talks to engine-api.
pub struct UiManager {
    windows: Vec<Box<dyn overlay_ui::Window + Send>>,
    engine_instance: engine_api::Engine,
...
impl UiManager {
    pub fn new(engine_instance: engine_api::Engine) -> Self {
        Self {
...
fn initialize_engine_and_app() {
    match engine_api::Engine::initialize() {
        Ok(instance) => {
            if OVERLAY_APP.set(Mutex::new(UiManager::new(instance))).is_err() {
...
/// Called by d3d9-present-hook on first valid device.
pub fn on_device_created(hwnd: HWND, device: &IDirect3DDevice9) {
    ...
    INIT.call_once(|| {
        ...
        let renderer = egui_d3d9::EguiDx9Lite::init(device, hwnd, false);
...
/// Called by d3d9-present-hook every Present.
    ...
    if let Some(mutex) = EGUI_RENDERER.get() {
        if let Ok(mut renderer) = mutex.lock() {
            renderer.present(device, |ctx| {
                if let Some(app) = OVERLAY_APP.get() {
...
/// Called by d3d9-present-hook before Reset.
pub fn on_pre_reset() {
    if let Some(mutex) = EGUI_RENDERER.get() {
        if let Ok(mut renderer) = mutex.lock() {
            renderer.pre_reset();
...
/// Called by d3d9-present-hook after Reset.
...
pub unsafe extern "stdcall" fn hooked_wndproc(
    ...
    // Route via UiManager
    let mut should_pass_to_game = true;
    if let Some(app_mutex) = OVERLAY_APP.get() {
...
pub fn uninstall_overlay() {
    ...
    // EGUI_RENDERER and OVERLAY_APP will be dropped with DLL, but we detach hooks now.
}
